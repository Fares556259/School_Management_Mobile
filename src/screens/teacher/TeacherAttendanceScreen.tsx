  ...
  return (
    <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 28, marginBottom: 16, borderWidth: 1, borderColor: '#f1f5f9', shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.02, shadowRadius: 15, elevation: 2 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 20 }}>
        ...
        <TouchableOpacity onPress={() => setShowNote(!showNote)} style={{ width: 44, height: 44, borderRadius: 14, backgroundColor: showNote ? '#eff6ff' : '#f8fafc', alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: showNote ? '#dbeafe' : '#f1f5f9' }}><MessageSquare size={20} color={showNote ? '#0055d4' : '#64748b'} /></TouchableOpacity>
      </View>
...
export const TeacherAttendanceScreen = ({ navigation }: any) => {
  ...
  const weekDates = Array.from({ length: 7 }, (_, i) => moment().startOf('week').add(i, 'days'));
  ...
  const loadClasses = async () => {
    try {
      ...
      setClasses(res);
      if (res.length > 0) { setSelectedClass(res[0]); loadStudents(res[0].id, selectedDate.format('YYYY-MM-DD')); }
