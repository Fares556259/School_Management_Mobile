        ...
        >
          ...
          >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <TouchableOpacity
                onPress={handleBack}
                style={{ width: 40, height: 40, borderRadius: 14, backgroundColor: '#f1f4f6', alignItems: 'center', justifyContent: 'center' }}
              >
                <ChevronLeft size={20} color="#2b3437" strokeWidth={3} style={{ transform: [{ rotate: isRTL ? '180deg' : '0deg' }] }} />
              </TouchableOpacity>

              {/* Language Selection */}
              <View style={{ flexDirection: 'row', backgroundColor: '#f8fafc', borderRadius: 16, padding: 3, borderWidth: 1, borderColor: '#e2e8f0' }}>
                {[
                  { id: 'ar', label: 'العربية 🇹🇳' },
                  ...
                  <TouchableOpacity
                    ...
                    style={{ paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, backgroundColor: language === item.id ? '#0055d4' : 'transparent' }}
                  >
                    <Text style={{ fontSize: 11, fontWeight: '800', color: language === item.id ? 'white' : '#64748b' }}>
                      {item.label}
                    </Text>
                  </TouchableOpacity>
                ))}
            ...
            <View style={{ alignItems: 'center', marginBottom: 32 }}>
              <View style={{ width: 76, height: 76, borderRadius: 24, backgroundColor: '#0055d4', alignItems: 'center', justifyContent: 'center', marginBottom: 16, shadowColor: '#0055d4', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.2, shadowRadius: 15, elevation: 8 }}>
            ...
            {/* Error Banner */}
            {!!error && (
              <View style={{ backgroundColor: '#fee2e2', borderRadius: 14, borderWidth: 2, borderColor: '#fca5a5', padding: 14, marginBottom: 16 }}>
                <Text style={{ color: '#dc2626', fontSize: 14, fontWeight: '800', textAlign: 'center' }}>{error}</Text>
              </View>
            )}

            {/* Hint Banner */}
            {!!hint && (
              <View style={{ backgroundColor: '#dcfce7', borderRadius: 14, borderWidth: 2, borderColor: '#86efac', padding: 14, marginBottom: 16 }}>
                <Text style={{ color: '#16a34a', fontSize: 14, fontWeight: '800', textAlign: 'center' }}>✓ {hint}</Text>
              </View>
            )}
            ...
            >
              {isLoading ? (
                <>
                  ...
                  <Text style={{ color: 'white', fontWeight: '700', fontSize: 16 }}>{loadingLabel}</Text>
                </>
              ) : (
                <>
                  <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 18 }}>{btnLabel}</Text>
